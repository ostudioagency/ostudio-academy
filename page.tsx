...
export default function AdminPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault()
    fetchEntries(secret, filterProfile, 1)
  }
        ...
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              ...
              <input
                type="password"
                className="w-full bg-obsidian-soft border border-cream/10 text-cream font-body text-sm px-4 py-3 placeholder:text-cream/20 focus:outline-none focus:border-gold transition-colors duration-200"
                placeholder="••••••••••••"
                value={secret}
            ...
            <button type="submit" disabled={loading || !secret} className="w-full btn-gold disabled:opacity-40">
      ...
      <header className="bg-obsidian border-b border-gold/15 px-6 md:px-8 py-5">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex flex-col">
            <span className="font-display text-lg font-light tracking-widest text-cream">O&apos;STUDIO</span>
            <span className="text-[8px] font-body tracking-widest uppercase text-gold">Academy · Admin</span>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => fetchEntries(secret, filterProfile, 1)} className="flex items-center gap-2 text-xs font-body tracking-widest uppercase text-cream/50 hover:text-gold transition-colors duration-200">
              <RefreshCw className="w-3 h-3" />
              <span className="hidden md:inline">Actualiser</span>
            </button>
            <button onClick={exportCSV} className="flex items-center gap-2 btn-outline border-gold/40 text-gold/70 text-xs px-4 py-2">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 py-10">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-10">
          ...
          {Object.entries(profileLabels).map(([key, label]) => (
            <div key={key} className="bg-white border border-obsidian/5 p-5 cursor-pointer hover:border-gold/30 transition-colors" onClick={() => handleFilter(filterProfile === key ? 'all' : key)}>
              <p className="text-[10px] font-body tracking-widest uppercase text-obsidian/40 mb-2">{label}</p>
              <p className="font-display text-3xl font-light text-obsidian">{stats[key as keyof typeof stats]}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3 mb-6 flex-wrap">
          ...
          {['all', ...Object.keys(profileLabels)].map((p) => (
            <button key={p} onClick={() => handleFilter(p)} className={`text-xs font-body tracking-widest uppercase px-3 py-1.5 border transition-all duration-200 ${filterProfile === p ? 'bg-obsidian text-cream border-obsidian' : 'text-obsidian/50 border-obsidian/15 hover:border-gold/50 hover:text-obsidian'}`}>
                  ...
                  {entries.map((entry) => (
                    <tr key={entry.id}>
                      <td className="px-6 text-obsidian/40 whitespace-nowrap">
                        {new Date(entry.created_at).toLocaleDateString('fr-CA', { month: 'short', day: 'numeric', year: 'numeric' })}
                      </td>
                      <td className="px-6 font-medium text-obsidian whitespace-nowrap">{entry.full_name}</td>
                      <td className="px-6 text-obsidian/60">
                        <a href={`mailto:${entry.email}`} className="hover:text-gold transition-colors duration-150">{entry.email}</a>
                      </td>
                      <td className="px-6 text-obsidian/50 hidden md:table-cell">{entry.phone || <span className="text-obsidian/20">—</span>}</td>
                      <td className="px-6 text-obsidian/50 hidden md:table-cell">{entry.city || <span className="text-obsidian/20">—</span>}</td>
                      <td className="px-6">
                        <span className={`inline-block text-[10px] font-body tracking-wide border px-2 py-0.5 ${profileColors[entry.profile] || 'bg-gray-50 text-gray-600 border-gray-200'}`}>
                      ...
                      <td className="px-6 hidden lg:table-cell">
                        <div className="flex flex-wrap gap-1">
                          {(entry.learning_goals || []).map((goal) => (
                            <span key={goal} className="text-[10px] font-body border border-obsidian/10 text-obsidian/50 px-1.5 py-0.5">{goal}</span>
                          ))}
                        </div>
                      </td>
                    </tr>
